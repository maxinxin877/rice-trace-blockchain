<template>
  <MobileShell title="扫码溯源" subtitle="一物一码 · 全程可信">
    <div class="query-card">
      ...
      <button class="sample" @click="useSample">使用演示码 {{ sampleCode }}</button>
    </div>
    ...
    <template v-else-if="detail">
      <section class="product-card">
        ...
        <el-tag :type="statusTag.type" effect="dark">{{ statusTag.label }}</el-tag>
      </section>
      <section class="verify-strip" :class="{ risky: isRisky }"><el-icon><CircleCheckFilled /></el-icon><div><strong>{{ isRisky ? '该防伪码存在风险' : isInactive ? '防伪码尚未激活' : '产品身份有效' }}</strong><p>{{ isRisky ? '检测到跨区域等异常扫码记录，请谨慎购买并核验销售渠道' : isInactive ? '产品尚未完成出库激活，不作为正品凭证' : '防伪码已激活，关键生产数据已上链存证' }}</p></div><span>扫码 {{ detail.traceCode.scanCount }} 次</span></section>
...
const sampleCode = import.meta.env.VITE_USE_MOCK === 'true' ? 'RC20260718000001' : 'RC20260917000001'
const route=useRoute(),router=useRouter(),code=ref(String(route.query.code||sampleCode)),detail=ref<TraceDetail|null>(null),loading=ref(false),searched=ref(false)
