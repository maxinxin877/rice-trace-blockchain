  ...
  <MobileShell title="扫码溯源" subtitle="一物一码 · 全程可信">
    ...
    <template v-else-if="detail">
      <section class="product-card">
        ...
        <el-tag :type="statusInfo.tagType" effect="dark">{{ statusInfo.tagText }}</el-tag>
      </section>
      <section :class="['verify-strip', statusInfo.stripClass]"><el-icon><component :is="statusInfo.icon" /></el-icon><div><strong>{{ statusInfo.stripTitle }}</strong><p>{{ statusInfo.stripDesc }}</p></div><span>扫码 {{ detail.traceCode.scanCount }} 次</span></section>
...
const statusInfo = computed(() => {
  const status = detail.value?.traceCode.status || 'GENERATED'
  switch (status) {
    case 'ACTIVATED':
      return { tagType: 'success', tagText: '已激活', stripClass: 'verify-ok', stripTitle: '产品身份有效', stripDesc: '防伪码已激活，关键生产数据已上链存证', icon: CircleCheckFilled }
    case 'DISABLED':
      return { tagType: 'danger', tagText: '已停用', stripClass: 'verify-risk', stripTitle: '该防伪码已停用', stripDesc: '请联系品牌方核实，谨防假冒产品', icon: CircleCloseFilled }
    default:
      return { tagType: 'warning', tagText: '未激活', stripClass: 'verify-warn', stripTitle: '防伪码尚未激活', stripDesc: '该产品尚未完成出库激活，请核对销售渠道', icon: WarningFilled }
  }
})
