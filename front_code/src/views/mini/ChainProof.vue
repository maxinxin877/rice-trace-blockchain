...
const route=useRoute(),code=String(route.query.code||'RC20260917000001'),proof=ref<ChainProofRecord|null>(null),loading=ref(false),verifying=ref(false)
...
async function verify(){verifying.value=true;try{proof.value=(await traceabilityApi.verifyMiniChainProof(code)).data;proof.value?.verified?ElMessage.success('链上核验通过'):ElMessage.error('链上核验异常')}finally{verifying.value=false}}
onMounted(load)
</script>
