...
const emptySummary: DashboardSummary = { fieldCount: 0, plantingBatchCount: 0, storedGrainWeightKg: 0, productBatchCount: 0, traceCodeCount: 0, normalTraceCodeCount: 0, riskTraceCodeCount: 0, scanCount: 0, riskWarningCount: 0, chainSuccessRate: 0, updatedAt: '' }
...
export default defineComponent({
  ...
  setup() {
    ...
    const riskOption = computed<EChartsOption>(() => ({ tooltip: { trigger: 'item' }, legend: { bottom: 0 }, series: [{ type: 'pie', radius: ['48%', '72%'], center: ['50%', '44%'], data: [{ name: '正常防伪码', value: summary.value.normalTraceCodeCount, itemStyle: { color: '#65a765' } }, { name: '风险防伪码', value: summary.value.riskTraceCodeCount, itemStyle: { color: '#c53030' } }] }] }))
    ...
    return () => h(PageContainer, { title: '数据看板' }, {
      ...
      default: () => [
        ...
        h('div', { class: 'dashboard-footnote' }, [h('span', '数据来源：GET /rice/dashboard/summary'), h(ElTag, { type: 'success' }, () => `更新至 ${summary.value.updatedAt || '-'}`)]),
      ],
    })
  },
})
